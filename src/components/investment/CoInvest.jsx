          ...
          <>
            {isOpen && (
              <div className="teams-close cursor-pointer hover:shadow-lg bg-black ">
                ...
                <button
                  className="flex text-lg font-bold border-2 border-solid border-gold py-3 px-[24px] text-white cursor-pointer md:text-xl hover:bg-primary_yellow hover:text-black transition-all duration-300"
                  onClick={() => handleInvest()}
                >
                  Co-invest
                </button>
                {showinvestwithus && (
                  <InvestWithUs onClose={() => handleCloseInvest()} />
                )}
              </div>
            )}
          </>
